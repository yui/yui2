...
(function() {
    ...
    YAHOO.util.Dom = {
        ...
        getBoxAttributes: function(el, side) { // assumes pixels
            side = side.charAt(0).toUpperCase() + side.substr(1).toLowerCase(); // e.g. "Left"
            var attributes = ['margin', 'border', 'padding', side];
            var box = {};
            var val;
            var attr;
            ...
            for (var i = 0, len = attributes.length; i < len; ++i) {
                switch(attributes[i]) {
                    case side:
                        attributes[i] =  attr = side.toLowerCase();
                        break;
                    case 'border':
                        attr =  attributes[i] + side + 'Width';
                        break;
                    default:
                        attr = attributes[i] + side; 
                }

                val = parseInt( Y.Dom.getStyle(el, attr) );

                if ( isNaN(val) ) { // in case of "auto" or invalid
                    val = 0;
                }

                box[attributes[i]] = val;
            } 
        ...
        getXY: function(el) {
            ...
            // has to be part of document to have pageXY
                ...
                if (el.getBoundingClientRect) { // IE
                    ...
                    // client starts at 2 unless there is a border on root
                    if (Y.Dom.getBoxAttributes(rootNode, 'left').border === 0) {
                        box.left -= 2;
                    }
                    if (Y.Dom.getBoxAttributes(rootNode, 'top').border === 0) {
                        box.top -= 2;
                    }
                    ...
                    // add HTML node's margin, padding, and border (assumes px)
                    if (!isSafari && el == document.body) {
                        var offsets = [
                            Y.Dom.getBoxAttributes(rootNode, 'left'),
                            Y.Dom.getBoxAttributes(rootNode, 'top')

                        ];
                        pos[0] += offsets[0].margin + offsets[0].padding + offsets[0].border; 
                        pos[1] += offsets[1].margin + offsets[1].padding + offsets[1].border; 
/*
                        if (isOpera && Y.Dom.getStyle(rootNode != 'static')) { // gecko ignores top/left etc.
                            pos[0] += offsets[0].left;
                            pos[1] += offsets[1].top;
                        }
*/
                    }
                    ...
                    if (isGecko || (isOpera && el == document.body)) { // && not opera ?
                        pos[0] += Y.Dom.getBoxAttributes(document.body, 'left').margin || 0;
                        pos[1] += Y.Dom.getBoxAttributes(document.body, 'top').margin || 0;
                    }
                }
