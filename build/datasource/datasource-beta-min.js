...
return this.makeConnection(oRequest,oCallback,oCaller);};YAHOO.util.DataSource.prototype.makeConnection=function(oRequest,oCallback,oCaller){this.fireEvent("requestEvent",{request:oRequest,callback:oCallback,caller:oCaller});var oRawResponse=null;var tId=YAHOO.util.DataSource._nTransactionId++;switch(this.dataType){case YAHOO.util.DataSource.TYPE_JSARRAY:case YAHOO.util.DataSource.TYPE_JSON:oRawResponse=this.liveData;this.handleResponse(oRequest,oRawResponse,oCallback,oCaller,tId);break;case YAHOO.util.DataSource.TYPE_JSFUNCTION:oRawResponse=this.liveData(oRequest);this.handleResponse(oRequest,oRawResponse,oCallback,oCaller,tId);break;case YAHOO.util.DataSource.TYPE_HTMLTABLE:oRawResponse=this.liveData;this.handleResponse(oRequest,oRawResponse,oCallback,oCaller,tId);break;case YAHOO.util.DataSource.TYPE_XHR:var oSelf=this;var oConnMgr=this.connMgr||YAHOO.util.Connect;var oConn=null;var allQueues=this._aQueue;var oQueue=null;for(var i=allQueues.length-1;i>-1;i--){if(allQueues[i].caller===oCaller){oQueue=allQueues[i];break;}}
if(!oQueue){oQueue={caller:oCaller,conn:null,requests:[]};allQueues.push(oQueue);}
...
else{this.handleResponse(oRequest,oResponse,oCallback,oCaller,tId);}};var _xhrFailure=function(oResponse){this.fireEvent("dataErrorEvent",{request:oRequest,callback:oCallback,caller:oCaller,message:YAHOO.util.DataSource.ERROR_DATAINVALID});oCallback.call(oCaller,oRequest,oResponse,true);return null;};var _xhrCallback={success:_xhrSuccess,failure:_xhrFailure,scope:this};if(YAHOO.lang.isNumber(this.connTimeout)){_xhrCallback.timeout=this.connTimeout;}
if(this.connXhrMode=="cancelStaleRequests"){if(oConnMgr.abort){if(oQueue.conn){oConnMgr.abort(oQueue.conn);oQueue.conn=null;}}
else{}}
var sUri=this.liveData+"?"+oRequest;if(oConnMgr&&oConnMgr.asyncRequest){if(this.connXhrMode!="queueRequests"){oConn=oConnMgr.asyncRequest("GET",sUri,_xhrCallback,null);if(this.connXhrMode!="allowAll"){oQueue.conn=oConn;}}
else{if(oQueue.conn){oQueue.requests.push(sUri);if(!oQueue.interval){oQueue.interval=setInterval(function(){if(oConnMgr.isCallInProgress(oQueue.conn)){return;}
else{if(oQueue.requests.length>0){oQueue.conn=oConnMgr.asyncRequest("GET",sUri,_xhrCallback,null);}
else{clearInterval(oQueue.interval);}}},50);}}
else{oQueue.conn=oConnMgr.asyncRequest("GET",sUri,_xhrCallback,null);}}}
...
oParsedResponse.tId=tId;oParsedResponse=this.doBeforeCallback(oRequest,oRawResponse,oParsedResponse);if(oParsedResponse){this.fireEvent("responseParseEvent",{request:oRequest,response:oParsedResponse,callback:oCallback,caller:oCaller});this.addToCache(oRequest,oParsedResponse);}
...
oCallback.call(oCaller,oRequest,oParsedResponse);};YAHOO.util.DataSource.prototype.doBeforeParseData=function(oRequest,oRawResponse){return oRawResponse;};YAHOO.util.DataSource.prototype.doBeforeCallback=function(oRequest,oRawResponse,oParsedResponse){return oParsedResponse;};YAHOO.util.DataSource.prototype.parseArrayData=function(oRequest,oRawResponse){if(YAHOO.lang.isArray(oRawResponse)&&YAHOO.lang.isArray(this.responseSchema.fields)){var oParsedResponse={results:[]};var fields=this.responseSchema.fields;for(var i=oRawResponse.length-1;i>-1;i--){var oResult={};for(var j=fields.length-1;j>-1;j--){var field=fields[j];var key=field.key||field;var data=oRawResponse[i][j]||oRawResponse[i][key];if(field.converter){data=field.converter(data);}
...
else{oParsedResponse.results=[];for(var k=xmlList.length-1;k>=0;k--){var result=xmlList.item(k);var oResult={};for(var m=this.responseSchema.fields.length-1;m>=0;m--){var field=this.responseSchema.fields[m];var key=field.key||field;var data=null;var xmlAttr=result.attributes.getNamedItem(key);if(xmlAttr){data=xmlAttr.value;}
...
for(var i=jsonList.length-1;i>=0;i--){var oResult={};var jsonResult=jsonList[i];for(var j=fields.length-1;j>=0;j--){var field=fields[j];var key=field.key||field;var data=eval("jsonResult."+key);if(field.converter){data=field.converter(data);}
if(data===undefined){data=null;}
oResult[key]=data;}
oParsedResponse.results.unshift(oResult);}}
else{oParsedResponse.error=true;}
return oParsedResponse;};YAHOO.util.DataSource.prototype.parseHTMLTableData=function(oRequest,oRawResponse){var bError=false;var elTable=oRawResponse;var fields=this.responseSchema.fields;var oParsedResponse={};oParsedResponse.results=[];for(i=0;i<elTable.tBodies.length;i++){elTbody=elTable.tBodies[i];for(var j=0;j<elTbody.rows.length;j++){var elRow=elTbody.rows[j];var oResult={};for(var k=fields.length-1;k>-1;k--){var field=fields[k];var key=field.key||field;var data=elRow.cells[k].innerHTML;if(field.converter){data=field.converter(data);}
if(data===undefined){data=null;}
oResult[key]=data;}
oParsedResponse.results.unshift(oResult);}}
if(bError){oParsedResponse.error=true;}
else{}
return oParsedResponse;};YAHOO.register("datasource",YAHOO.util.DataSource,{version:"@VERSION@",build:"@BUILD@"});