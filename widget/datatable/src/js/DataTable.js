    ...
    this.setAttributeConfig("scrollable", {
        ...
        method: function(oParam) {
            if(oParam) {
                ...
                //YAHOO.util.Event.addListener(this._elContainerLiner, 'scroll', this._onContainerLinerScroll, this);

                // Unset snapped width
                //this._elContainer.style.width = "";
                // Set explicit padding
                if(!YAHOO.env.ua.ie) {
                    this._elContainer.style.paddingBottom = this._elThead.offsetHeight + "px"; //"24px";
                }

                /*if(YAHOO.env.ua.ie) {
                    YAHOO.util.Dom.setStyle(this._elContainer, "width", this._elTable.offsetWidth);
                }
                // For gecko bug https://bugzilla.mozilla.org/show_bug.cgi?id=135236
                if(YAHOO.env.ua.gecko) {
                    YAHOO.util.Dom.addClass(this._elTable,YAHOO.widget.DataTable.CLASS_SCROLLTABLE);
                }
                ...
                if(YAHOO.env.ua.opera || YAHOO.env.ua.webkit) {
                    YAHOO.util.Dom.addClass(this._elTable,YAHOO.widget.DataTable.CLASS_SCROLLBLOCK);
                    this.bFixedScrollBlockWorkaround = true;
                }*/
            }
            else {
                YAHOO.util.Dom.removeClass(this._elContainer,YAHOO.widget.DataTable.CLASS_SCROLLABLE);
                //YAHOO.util.Event.removeListener(this._elContainerLiner, 'scroll', this._onContainerLinerScroll);

                // Snap container to width
                //this._elContainer.style.width = this._elTable.offsetWidth + "px";
                // Unset explicit padding
                //this._elContainer.style.paddingTop = "";
                if(!YAHOO.env.ua.ie) {
                    this._elContainer.style.paddingBottom = ""; //"24px";
                }
...
/**
 * DOM reference to the container element for the DataTable header.
 *
 * @property _elHeadContainer
 * @type HTMLElement
 * @private
 */
YAHOO.widget.DataTable.prototype._elHeadContainer = null;

/**
 * DOM reference to the container element for the DataTable body.
 *
 * @property _elBodyContainer
 * @type HTMLElement
 * @private
 */
YAHOO.widget.DataTable.prototype._elBodyContainer = null;
...
/**
 * DOM reference to the primary TABLE element for the DataTable instance.
 *
 * @property _elTable
 * @type HTMLElement
 * @private
...
*/
    ...
    for(i=0; i<this._oColumnSet.keys.length; i++) {
        ...
        nWidth = Math.min(elHeadCell.offsetWidth, elBodyCell.offsetWidth);
        ...
        if(elBodyCell.offsetWidth > elHeadCell.offsetWidth) {
            nWidth = elHeadCell.offsetWidth;
            while(elBodyCell.offsetWidth > elHeadCell.offsetWidth) {
                nWidth ++;
                elHeadCell.style.width = nWidth + "px";
            }
        }
        else if(elBodyCell.offsetWidth < elHeadCell.offsetWidth) {
            nWidth = elBodyCell.offsetWidth;
            while(elBodyCell.offsetWidth < elHeadCell.offsetWidth) {
                nWidth ++;
                elBodyCell.style.width = nWidth + "px";
            }
    ...
    /*for(i=0; i<this._oColumnSet.keys.length; i++) {
        ...
        if(elBodyCell.offsetWidth > elHeadCell.offsetWidth) {
            elHeadCell.style.width = elBodyCell.offsetWidth + "px";
            ...
            elBodyCell.style.width = elHeadCell.offsetWidth + "px";
...
YAHOO.widget.DataTable.prototype._initContainerEl = function(elContainer) {
    ...
    if(elContainer && elContainer.tagName && (elContainer.tagName.toLowerCase() == "div")) {
        ...
        this._elHeadContainer = elContainer.appendChild(document.createElement("div"));
        YAHOO.util.Dom.addClass(this._elHeadContainer, "yui-dt-hd");
        
        // Container for header elements
        this._elBodyContainer = elContainer.appendChild(document.createElement("div"));
        YAHOO.util.Dom.addClass(this._elBodyContainer, "yui-dt-bd");

        //YAHOO.util.Event.addListener(elContainerLiner, "scroll", this._onScroll, this); // for IE
...
YAHOO.widget.DataTable.prototype._initTableEl = function() {
    // Destroy existing
    if(this._elTable) {
        YAHOO.util.Event.purgeElement(this._elTable, true);
    ...
    this._elHeadTable = this._elHeadContainer.appendChild(elHeadTable);
    ...
    this._elTable = this._elBodyContainer.appendChild(elBodyTable);
    ...
    var elBodyContainer = this._elBodyContainer;

    // Set up DOM events
    YAHOO.util.Event.addListener(document, "click", this._onDocumentClick, this);
    YAHOO.util.Event.addListener(document, "keydown", this._onDocumentKeydown, this);

    YAHOO.util.Event.addListener(elBodyTable, "focus", this._onTableFocus, this);
    YAHOO.util.Event.addListener(elBodyTable, "mouseover", this._onTableMouseover, this);
    YAHOO.util.Event.addListener(elBodyTable, "mouseout", this._onTableMouseout, this);
    YAHOO.util.Event.addListener(elBodyTable, "mousedown", this._onTableMousedown, this);
    YAHOO.util.Event.addListener(elBodyTable, "keydown", this._onTableKeydown, this);
    YAHOO.util.Event.addListener(elBodyTable, "keypress", this._onTableKeypress, this);

    // Since we can't listen for click and dblclick on the same element...
    YAHOO.util.Event.addListener(elBodyTable, "dblclick", this._onTableDblclick, this);
    ...
    YAHOO.util.Event.addListener(elBodyContainer, "scroll", this._onScroll, this); // to sync horiz scroll headera
};
...
YAHOO.widget.DataTable.prototype._initTheadEl = function(elTable, bA11y) {
    ...
    //TODO: Create before the CAPTION in Safari

    // Create COLs
    // Iterate through each Column key...
    /*var colKeys = oColumnSet.keys;
    this._elColGroup = elTable.appendChild(document.createElement("colgroup"));
    for(i=0; i<colKeys.length; i++) {
        oColumn = colKeys[i];
        var elCol;
        elCol = oColumn._elCol = this._elColGroup.appendChild(document.createElement("col"));
        elCol.id = this.id + "-col" + oColumn.getId();
        YAHOO.util.Dom.addClass(elCol, "yui-dt-col-"+oColumn.getKey());
        //TODO: Traverse up Column parents to assign _elCol
    }*/
    ...
    for(i=0; i<aFirstHeaders.length; i++) {
        ...
        YAHOO.util.Dom.addClass(YAHOO.util.Dom.get(this.id+"-th"+aFirstHeaders[i]), YAHOO.widget.DataTable.CLASS_FIRST);
    }
    for(i=0; i<aLastHeaders.length; i++) {
        //TODO: A better way to get th cell
        YAHOO.util.Dom.addClass(YAHOO.util.Dom.get(this.id+"-th"+aLastHeaders[i]), YAHOO.widget.DataTable.CLASS_LAST);
    }
...
YAHOO.widget.DataTable.prototype._addTrEl = function(oRecord, index) {
    ...
    for(var j=0; j<oColumnSet.keys.length; j++) {
        ...
        for(var k=0; k<oColumnSet.headers[j].length; k++) {
            elCell.headers += this.id + "-th" + oColumnSet.headers[j][k] + " ";
        }
...
YAHOO.widget.DataTable.prototype._onScroll = function(e, oSelf) {
    /*for(var i=0; i<oSelf._oColumnSet.keys.length; i++) {
        var elLiner = oSelf._oColumnSet.keys[i].getThEl().firstChild;
        elLiner.style.position = "";
    }*/
    oSelf._elHeadContainer.scrollLeft = oSelf._elBodyContainer.scrollLeft;
    /*for(i=0; i<oSelf._oColumnSet.keys.length; i++) {
        elLiner = oSelf._oColumnSet.keys[i].getThEl().firstChild;
        elLiner.style.position = "relative";
    }*/
...
YAHOO.widget.DataTable.prototype.getNextTrEl = function(row) {
    ...
    YAHOO.log("Could not get next TR element for row " + row, "warn", this.toString());
    return null;
};
...
YAHOO.widget.DataTable.prototype.getPreviousTrEl = function(row) {
    ...
    YAHOO.log("Could not get previous TR element for row " + row, "warn", this.toString());
    return null;
};
...
YAHOO.widget.DataTable.prototype.getFirstTdEl = function(row) {
    ...
    YAHOO.log("Could not get first TD element for row " + elRow, "warn", this.toString());
    return null;
};
...
YAHOO.widget.DataTable.prototype.getLastTdEl = function(row) {
    ...
    YAHOO.log("Could not get last TD element for row " + elRow, "warn", this.toString());
    return null;
};
...
YAHOO.widget.DataTable.prototype.getNextTdEl = function(cell) {
    ...
    YAHOO.log("Could not get next TD element for cell " + cell, "warn", this.toString());
    return null;
};
 ...
 */
YAHOO.widget.DataTable.prototype.getPreviousTrEl = function(cell) {
    var elCell = this.getTdEl(cell);
    if(elCell) {
        var nThisTdIndex = elCell.yuiCellIndex;
        var elRow = this.getTrEl(elCell);
...
YAHOO.widget.DataTable.prototype.sortColumn = function(oColumn) {
    if(oColumn && (oColumn instanceof YAHOO.widget.Column)) {
        ...
        if(this._elContainerLiner.scrollTop > 0) {
            nOrigScrollTop = this._elContainerLiner.scrollTop;
            this._elContainerLiner.scrollTop = 0;
        }*/
        ...
        /*if(nOrigScrollTop !== null) {
            this._elContainerLiner.scrollTop = nOrigScrollTop;
        }*/
...
YAHOO.widget.DataTable.prototype.hideColumn = function(oColumn) {
    ...
    if(oColumn && !oColumn.hidden) {
        ...
        var l = allrows.length;
        for(i=0;i<l;i++) {
            allrows[i].cells[nColKeyIndex].firstChild.style.margin = 0;
            allrows[i].cells[nColKeyIndex].firstChild.style.padding = 0;
        }
        ...
        // Store for later
        oColumn.width = elLiner.offsetWidth;
        oColumn.hidden = true;
        // Opera and Safari don't like 0px
        if(!YAHOO.env.ua.opera && !YAHOO.env.ua.webkit) {
            this.setColumnWidth(oColumn, "0px");
        }
        else {
            this.setColumnWidth(oColumn, "1px");
        }

        // Adjust any parents of nested header ONLY
        // if this is the last visible child getting hidden
        var oParent = oColumn.parent;
        while(oParent) {
            var nVisibleChildren = oParent.children.length;
            for(i=0; i<oParent.children.length; i++) {
                if(oParent.children[i].hidden) {
                    nVisibleChildren--;
                }
            }
            if(nVisibleChildren === 1) {
                ...
                // Store for later
                oParent.width = elLiner.offsetWidth;
                oParent.hidden = true;

                if(!YAHOO.env.ua.opera && !YAHOO.env.ua.webkit) {
                    oParent.getThEl().firstChild.style.width = "0px";
                }
                else {
                    oParent.getThEl().firstChild.style.width = "1px";
                }
            }
...
YAHOO.widget.DataTable.prototype.showColumn = function(oColumn) {
    ...
    if(oColumn && oColumn.hidden) {
        ...
        this.setColumnWidth(oColumn, oColumn.width + "px");
        ...
        var l = allrows.length;
        for(var i=0;i<l;i++) {
            allrows[i].cells[nColKeyIndex].firstChild.style.margin = "";
            allrows[i].cells[nColKeyIndex].firstChild.style.padding = "";
        }
        ...
        // Adjust any parents of nested headers
