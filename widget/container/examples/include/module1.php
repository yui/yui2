			...
			<p>Next, we will build the JavaScript for our Modules, and wrap it in a function that will be executed when the window is finished loading. The Module called "module1" will be associated with our existing markup, and "module2" will be created dynamically from script. In this tutorial, we only pass the Module constructor the one required argument: the id associated with that Module's container.</p>
			...
			<p>To see our newly created Modules more easily, we can add a style block that defines custom CSS for the "module" CSS class. By default, Module doesn't come with any predefined styles, so it is up to the developer to provide any applicable styles. Our style block will display the Modules and each of their child elements with distinct border colors so that they are easy to discern from one another:</p>
			...
			<p>Finally, we will add some HTML buttons to the page that will take advantage of the <em>show</em> and <em>hide</em> methods provided by Module:</p>
