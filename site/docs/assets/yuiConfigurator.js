...
YAHOO.yui.configurator = function(container) {
    ...
    function showDependencies() {
		...
		if(reqs.length) {
			var configUrl = "/yui/articles/hosting/?" + reqs.join("&");
			if (filter) {
				configUrl += "&" + filter;
			}
			if (loadOptional) {
    ...
    function init() {
        ...
        loadOptionalButton = new Button({
			...
			checked: ((qs.indexOf("loadOptional") > -1) ? true : false)
        });
        ...
        comboHandlerButton = new Button({
            ...
            label: "Combine Files",
            value: "true",
			checked: ((qs.indexOf("nocombine") > -1) ? false : true),
            container: "loadOptionalContainer" 
        });
        ...
        allowRollupButton = new Button({
            ...
            label: "Allow Rollup",
            value: "true",
            checked: ((qs.indexOf("norollup") > -1) ? false : true),
