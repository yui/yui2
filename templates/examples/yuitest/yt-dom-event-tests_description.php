...
<p>Next, the <code>setUp()</code> and <code>tearDown()</code> methods are included. The <code>setUp()</code> method is used in a <code>TestCase</code>
  to setup data that may be needed for tests to be completed. This method is called immediately before each test is executed. For this example,
  ...
  run immediately after each test is run and, in this case, deletes the data object that was created by <code>setUp</code>. These methods are optionall.</p>
...
<p>The first test in this object is <code>testName()</code>, which runs
  various assertions on <code>data.name</code>. Inside of this method, a shortcut to <code>YAHOO.util.Assert</code> is setup and used to run three
