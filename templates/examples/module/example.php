...
if (!isset($yuiCurrentVersion)) {
   if (isset($_GET['v'])) {
        ...
        echo "<h1>YUI Version Not Set</h1>";
        die; 
   }
}
...
if(! $examples[$name]) {
	...
	//in-context template:
		...
		if(isset($currentModuleName)&&isset($name)) {//header appears on some non-example pages, too
			$filename = $docroot."examples/".$currentModuleName."/".$name."_customheader.php";
...
';
    ...
    <?php if (isset($currentExample["byline"])) { ?>
        <cite class="byline"><?php echo $currentExample["byline"]; ?></cite>
    <?php } ?>
	...
	<?php 
	$filename = $docroot."examples/".$currentModuleName."/".$name."_intro.php";
	include($filename);
	?>			
	</div>	
...
?>		<div class="hd exampleHd">
	...
	<?php 
	$filename = $docroot."examples/".$currentModuleName."/".$name."_source.php";
	include($filename);
	?>
	...
	<?php 
	$filename =  $docroot."examples/".$currentModuleName."/".$name."_description.php";
	include($filename);
	
	//set up Configurator link:
	$exampleReqs = implode("&", $examples[$name]["requires"]);
	?>

<h2>Configuration for This Example</h2>

<p>You can load the necessary JavaScript and CSS for this example from Yahoo's servers.  <a href="http://developer.yahoo.com/yui/articles/hosting/?<?php echo $exampleReqs; ?>&MIN#configure">Click here to load the YUI Dependency Configurator with all of this example's dependencies preconfigured</a>.</p>
